...
<input type="button" value="Add pack"
	onclick="window.location.href='addpaid'; return false;" class="btn btn-primary btn-sm mb-3" style="position: absolute; margin-left: 1265px; margin-top:10px;" />
</div>			   
<form:form action="psearch" method="GET">
...
<c:url var="updatelink" value="/updatepaid">
<c:param name="id" value="${load.id}"></c:param>
</c:url>
<c:url var="deletelink" value="/deletepaid">
<c:param name="id" value="${load.id}"></c:param>
</c:url>
...
<table id="tload">
<tr>
<th> Total Packs = <%=session.getAttribute("ppacks") %></th>
<th> Total Amount(Rs) = <%= session.getAttribute("ptotal") %></th>
</tr>
</table>
</body>
</html>